...
class ChunkAuditorSliceTool(Tool):
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage]:
        ...
        if isinstance(doc_slices_text, dict):
            chunk_payload = doc_slices_text
        else:
            chunk_payload = safe_json_load(doc_slices_text, None)
        if isinstance(chunk_payload, str):
            chunk_payload = safe_json_load(chunk_payload, None)
        ...
        if isinstance(chunk_payload, dict) and isinstance(chunk_payload.get("chunks"), list):
            ...
            chunk_payload = chunks[0] if chunks else {}

        if not isinstance(chunk_payload, dict):
            for m in dual_messages(self, "Error: doc_slices_text must be a single chunk JSON object.", {"error": "Invalid doc_slices_text"}):
                yield m
                return
        ...
        chunk_text = str(chunk_payload.get("text", ""))
        element_refs = chunk_payload.get("element_refs", [])
        element_meta = chunk_payload.get("element_meta", [])
        chunk_hash = str(chunk_payload.get("chunk_hash", "")).strip()
        rules = rules_payload.get("rules", [])

        if not chunk_text.strip() or not rules:
        ...
        if output_language == "auto":
            output_language = detect_text_language(chunk_text)
