...
class DocAuditTemplateTool(Tool):
    @staticmethod
    def _get_subtool_class(operator: str) -> type[Tool]:
        ...
        if operator == "template_doc_auditor":
            from tools.template_doc_auditor import TemplateDocAuditorTool

            return TemplateDocAuditorTool
    ...
    def _invoke(self, tool_parameters: dict[str, Any]) -> Generator[ToolInvokeMessage]:
        ...
        try:
            ...
            template_audit_result = self._run_subtool(
                self._get_subtool_class("template_doc_auditor"),
                {
                    "model_config": llm_model,
                    "template_text": str(template_chunk.get("text", "")),
                    "doc_text": str(doc_chunk.get("text", "")),
                    "chunk_id": doc_chunk.get("chunk_id", 0),
            ...
            if template_audit_result.get("error"):
                for m in self._emit_error(step_index, step_name, str(template_audit_result.get("error"))):
                    yield m
                return
            ...
            revision_result = self._run_subtool(
                ...
                {
                    "model_config": llm_model,
                    "upload_file": self._blob_to_file(annotate_blobs[0], "reviewed.docx"),
            ...
            detail = str(e)
            logger.exception("Doc template audit failed: %s", detail)
